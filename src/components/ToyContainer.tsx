...
export default function SetupContainer(props: { channel: number }) {
  ...
  function RenderUI() {
    if (selectToy() == true) {
      return <div>{RenderToySelection()}</div>;
    } else {
      if (toyType() < 1) {
        return <div>{RenderToySelection()}</div>;
      } else {
  ...
  function RenderSpecificUISettings() {
    if (toyType() == 0) return <></>;
      ...
      switch (toyType()) {
        case 1:
          return (
            <SpecificUI.GraviBoardUI
              channel={channel}
            ></SpecificUI.GraviBoardUI>
          );
        case 2:
          return (
            <SpecificUI.PolyDrumUI channel={channel}></SpecificUI.PolyDrumUI>
          );
        case 3:
          return (
            <SpecificUI.MIDIMatrixUI
              channel={channel}
            ></SpecificUI.MIDIMatrixUI>
          );

        default:
          return <h3>Sorry, nothing</h3>;
      }
    }
  }
